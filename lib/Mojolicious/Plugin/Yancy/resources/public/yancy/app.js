...
Vue.component( 'foreign-key-field', {
    ...
    data: function () {
        ...
        return {
            ...
            displayField: this.$props.displayField || (
                schema['x-list-columns'] ? schema['x-list-columns'][0] : ( schema['x-id-field'] || 'id' )
            ),
    ...
    methods: {
        ...
        updateDisplayValue: function ( item ) {
            this.displayValue = item[ this.$data.displayField ];
            this.loading = false;
        },
        ...
        submitSearch: function () {
            $.ajax({
                method: 'GET',
                url: specUrl + '/' + this.$props.schemaName +
                    '?' + this.$data.displayField + '=' + encodeURIComponent( this.searchQuery ),
                context: this
            })
        ...
        updateSearchResults: function ( data ) {
            ...
            for ( var i = 0; i < data.items.length; i++ ) {
                results.push({
                    display: data.items[i][ this.$data.displayField ],
