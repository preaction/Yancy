...
configure the API from the database schema:
  ...
  # hide the "mojo_migrations" collection, which is not useful
  $ echo >yancy.conf "{backend=>'$TEST_ONLINE_SQLITE', read_schema=>1, collections => { mojo_migrations => {'x-hidden'=>1} }}"
